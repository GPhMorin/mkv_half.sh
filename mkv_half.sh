...
do
  ...
  if [[ "$(file $file)" == *"video: Motion JPEG"* ]]; then
    ...
    ffmpeg -loglevel error -i $file -f matroska -qp 0 -map 0:v:1 -map 0:a:0 "$filenamebase"_r.MKV
    echo Create left video: "$filenamebase"_l.MKV
    ffmpeg -loglevel error -i $file -f matroska -qp 0 -map 0:v:0 -map 0:a:0 "$filenamebase"_l.MKV
    ...
    ffmpeg -loglevel error -i "$filenamebase"_l.MKV -i "$filenamebase"_r.MKV \
	   ...
	   -f matroska -pix_fmt yuv420p -b:v 28454K -aspect 32:9 -metadata:s:v stereo_mode=left_right -c:a copy -map 0:a:0 "$filenamebase"_HSBS.MKV
  fi
done
